...
| GET/POST | `/login` | Вхід (з flash-повідомленням при невірному email/паролі) |
| POST | `/logout` | Вихід (через форму на сторінці профілю) |
| GET | `/profile` | Профіль (перегляд даних, форма logout) |
| POST | `/profile/change-password` | Повноцінна зміна пароля (перевірка старого пароля, підтвердження, мін. 8 символів) |
